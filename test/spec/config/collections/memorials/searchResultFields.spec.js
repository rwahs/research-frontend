        ...
        function (chai, searchResultFields) {
            ...
            describe('The `memorials/searchResultFields` module', function () {
                ...
                it('Defines the `Media` field', function () {
                    expect(searchResultFields[6].key).to.equal('Media');
                    expect(searchResultFields[6].labelText).to.equal('Image');
                    expect(searchResultFields[6].display).to.equal('image');
                    expect(searchResultFields[6].placeholder).to.equal('(No image available)');
                });
            });
        }
    );
