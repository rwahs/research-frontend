        ...
        function (chai, searchResultFields) {
            ...
            describe('The `library/searchResultFields` module', function () {
                ...
                it('Defines the `Title` field', function () {
                    expect(searchResultFields[0].key).to.equal('Title');
                    expect(searchResultFields[0].labelText).to.equal('Title');
                });
                it('Defines the `Author` field', function () {
                    expect(searchResultFields[1].key).to.equal('Author');
                    expect(searchResultFields[1].labelText).to.equal('Author');
                    expect(searchResultFields[1].parse).to.equal(true);
                    expect(searchResultFields[1].filter).to.equal(true);
                    expect(searchResultFields[1].display).to.equal('list');
                });
                it('Defines the `Publisher` field', function () {
                    expect(searchResultFields[2].key).to.equal('Publisher');
                    expect(searchResultFields[2].labelText).to.equal('Publisher');
                });
                it('Defines the `DateOfPublication` field', function () {
                    expect(searchResultFields[3].key).to.equal('DateOfPublication');
                    expect(searchResultFields[3].labelText).to.equal('Date of Publication');
                });
                it('Defines the `PlaceOfPublication` field', function () {
                    expect(searchResultFields[4].key).to.equal('PlaceOfPublication');
                    expect(searchResultFields[4].labelText).to.equal('Place of Publication');
                });
                it('Defines the `PublicationType` field', function () {
                    expect(searchResultFields[5].key).to.equal('PublicationType');
                    expect(searchResultFields[5].labelText).to.equal('Type');
                });
